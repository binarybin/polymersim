...
template <> vector<tuple<int, int, Pos2d1l>> CornerMove<Space2D1L, Pos2d1l>::GetPossibleMoves(Polymer<Pos2d1l>& poly)
{
    vector<tuple<int, int, Pos2d1l>> possible_moves;
        ...
        if (x1 != x3 && y1 != y3)
        {
            if (x1 == x2)
            {
                ...
                if (space.space[x3][y1] == 0)
                    possible_moves.push_back(make_tuple(pos+1, pos+1, Pos2d1l(x3, y1)));
            }
            else
            {
                assert(y1 == y2 && x2 == x3 && y2 != y3);
                if (space.space[x1][y3] == 0)
                    possible_moves.push_back(make_tuple(pos+1, pos+1, Pos2d1l(x1, y3)));
            }
        }
    }
